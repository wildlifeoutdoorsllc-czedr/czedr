...
Everything runs on your **Windows PC** — **Android emulator**, **phone on Wi‑Fi**, or **iPhone Safari**. No hosted API required yet.

1. DB: run `scripts\install-backend.ps1` (creates **saturn** only) + `.env` — see `docs/TEST-ACCOUNTS.md`
2. Start API: `scripts\start-php-server.ps1` (listens on **0.0.0.0:8080** — works for **Android emulator**, **phones on Wi‑Fi**, and **Safari**). `start-iphone-sandbox.ps1` is the same server.
...
2. Start API: `scripts\start-php-server.ps1`
